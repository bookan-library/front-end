...
export const newsletterStoreSlice: StateCreator<
  AppState,
  [],
  [],
  NewsletterStore
> = (set, get) => ({
  ...
  subscribeToNewsletter: async (email: string) => {
    ...
    try {
      const res = await axios.post(
        `${process.env.REACT_APP_URL}/newsletter/subscribe`,
        {
      ...
      toast.error("Error when subscribing to newsletter!", {
        position: toast.POSITION.BOTTOM_CENTER,
      });
    }
  },
  sendNewsletter: async (newsletter: Newsletter) => {
    ...
    try {
      ...
      const res = await axios.post(
        `${process.env.REACT_APP_URL}/newsletter/send`,
        body,
        {
          headers: {
      ...
      toast.error("Error when sending newsletter!", {
        position: toast.POSITION.BOTTOM_CENTER,
      });
    }
  },
