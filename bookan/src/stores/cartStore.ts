...
export const cartStoreSlice: StateCreator<AppState, [], [], CartStore> = (set, get) => ({
    ...state,
    addToCart: async (bookId: number, quantity: number) => {
        ...
        try {
            const res = await axios.post(`${process.env.REACT_APP_URL}/cart/add`,
                {
    ...
    getUserCart: async () => {
        ...
        try {
            const res = await axios.get(`${process.env.REACT_APP_URL}/cart/${get().loggedUser.data[0].id}`,
                {
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': 'Bearer ' + get().token
    ...
    updateInCart: async (quantity: number, bookId: number) => {
        ...
        try {
            const res = await axios.post(`${process.env.REACT_APP_URL}/cart/update`,
                {
                    buyerId: get().loggedUser.data[0].id,
                    bookId: bookId,
                    quantity: quantity
    ...
    removeFromCart: async (cartItemId: number) => {
        ...
        try {
            const res = await axios.delete(`${process.env.REACT_APP_URL}/cart/${cartItemId}/remove`,
                {
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': 'Bearer ' + get().token
