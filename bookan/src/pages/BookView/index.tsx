...
export const BookView = () => {
  ...
  const getBooksByCategory = useApplicationStore(
    (state) => state.getBooksByCategory
  );
  ...
  const updateUrlWithParams = () => {
    const publishersQuery = selectedPublishers
      ?.map((pub) => `Publishers=${pub}`)
      .join("&");
    navigate(
      `${location.pathname}?minPrice=${sliderValue[0]}&maxPrice=${sliderValue[1]}&${publishersQuery}`
    );
  };
  ...
  useEffect(() => {
    searchBooks(search, 1, params.category ?? "");
  }, [search]);
          ...
          {books?.data.map((book) => (
            ...
            >
              <Button
                ...
                onClick={() =>
                  navigate(
                    `/books/categories/${params.category ?? ""}/${book.id}`
                  )
                }
              >
                DETALJNIJE
