...
export const DetailedBookView = () => {
  ...
  const addToWishlistRes = useApplicationStore(
    (state) => state.addToWishlistRes
  );
  const loggedUser = useApplicationStore((state) => state.loggedUser.data[0]);
  const checkIfBookInWishlist = useApplicationStore(
    (state) => state.checkIfBookInWishlist
  );
  const isBookInWishlist = useApplicationStore(
    (state) => state.isBookInWishlist
  );
  const removeFromWishlist = useApplicationStore(
    (state) => state.removeFromWishlist
  );
  const removeFromWishlistRes = useApplicationStore(
    (state) => state.removeFromWishlistRes
  );
  ...
  useEffect(() => {
    ...
    console.log("wishes ", isBookInWishlist.data);
  }, []);
  ...
  return (
    ...
    >
      <Flex width={"100%"} gap={"5%"} justifyContent={"center"}>
        <Flex height={"400px"} justifyContent={"center"} w="30%">
          <img src={book.picUrl} />
        </Flex>
        <Flex direction={"column"} w="40%">
          <Text fontSize={"30px"}>{book.name.toUpperCase()}</Text>
          <Text color={"darkgray"} mt={"10px"}>
            {book?.category?.name}
          </Text>
          <Text color={"gray"} mt={"15px"}>
          ...
          <Text color={"red"} fontSize={"24px"} mt={"15px"}>
            {book.price} RSD
          </Text>
          ...
          {isBookInWishlist.data.length !== 0 && isBookInWishlist.data ? (
            <Flex
              alignItems={"center"}
              gap={"5px"}
              cursor={"pointer"}
      ...
      <Flex width={"100%"} justifyContent="center">
        ...
        >
          ...
          <TabPanels display={"flex"} justifyContent={"center"} width={"100%"}>
            ...
            <TabPanel>
              <Table variant="striped" width={"800px"} minW={"800px"}>
                <Tbody>
                  <Tr>
                    <Td>KATEGORIJA</Td>
            ...
            <TabPanel width={"80%"}>
              ...
              <Text mt={"20px"} fontSize={"18px"} ml={"10px"} color={"gray"}>
                OSTALI KOMENTARI
              </Text>
              <Divider />
