...
export const WishlistView = () => {
  ...
  const getWishlistCount = useApplicationStore(
    (state) => state.getWishlistCount
  );
    ...
    <Flex width={"100%"} direction={"column"} alignItems={"center"}>
      ...
      >
        ...
        <Box color={"gray"}>
          <Text>Adresa</Text>
          <Text>
            {loggedUser.data[0].address.street}{" "}
            {loggedUser.data[0].address.streetNumber},{" "}
            {loggedUser.data[0].address.city}
          </Text>
        </Box>
      </Flex>
      ...
      <Box width={"70%"} padding={"15px"}>
        ...
        <Text color={"gray"}>
          {loggedUser.data[0].firstName}, ovo je vasa lista
        </Text>
      </Box>
      ...
      <Paginator
        totalCount={wishlistCount.data as unknown as number}
