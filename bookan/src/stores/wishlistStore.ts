...
export const wishlistStoreSlice: StateCreator<
  AppState,
  [],
  [],
  WishlistStore
> = (set, get) => ({
  ...
  addToWishlist: async (bookId: number) => {
    ...
    try {
      const res = await axios.post(
        `${process.env.REACT_APP_URL}/wishlist/add`,
        {
      ...
      toast.error("Error when adding book to wishlist!", {
        position: toast.POSITION.BOTTOM_CENTER,
      });
    }
  },
  removeFromWishlist: async (bookId: number) => {
    ...
    try {
      const res = await axios.delete(
        `${process.env.REACT_APP_URL}/wishlist/${bookId}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: "Bearer " + get().token,
      ...
      toast.error("Error when removing book from wishlist!", {
        position: toast.POSITION.BOTTOM_CENTER,
      });
    }
  },
  getWishlist: async (pageNumber: number) => {
    ...
    try {
      const res = await axios.get(
        ...
        }/wishlist?pageNumber=${pageNumber}&buyerEmail=${
          get().loggedUser.data[0].email
        }`,
        {
          headers: {
            "Content-Type": "application/json",
  ...
  checkIfBookInWishlist: async (bookId: number) => {
    ...
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_URL}/wishlist/check/${bookId}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: "Bearer " + get().token,
