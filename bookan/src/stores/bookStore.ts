...
export const bookStoreSlice: StateCreator<AppState, [], [], BookStore> = (
  set,
  get
) => ({
  ...state,
  addBook: async (book: AddBook) => {
    ...
    try {
      ...
      toast.error("Error when adding book!", {
        position: toast.POSITION.BOTTOM_CENTER,
      });
    }
  },
  getBooksByCategory: async (
    ...
    try {
      ...
      const publishersQuery = params?.publishers
        ?.map((pub) => `Publishers=${pub}`)
        .join("&");
      const res = await axios.get(
        `${process.env.REACT_APP_URL}/books/categories/${category}?PageNumber=${pageNumber}&MinPrice=${params?.minPrice}&MaxPrice=${params?.maxPrice}&${publishersQuery}`,
        {
          headers: {
  ...
  searchBooks: async (search: string, pageNumber: number, category: string) => {
    set(
      produce((state: AppState) => {
        state.books.status = ResponseStatus.Loading;
        return state;
    ...
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_URL}/books?category=${category}&search=${search}&pageNumber=${pageNumber}`,
        {
          headers: {
            "Content-Type": "application/json",
          },
  ...
  getBookCount: async (category: string, params?: QueryParams) => {
    ...
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_URL}/books/${category}/count?MinPrice=${params?.minPrice}&MaxPrice=${params?.maxPrice}&Publishers=${params?.publishers}`,
        {
          headers: {
            "Content-Type": "application/json",
          },
